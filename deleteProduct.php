    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($row = mysqli_fetch_array($sql)) {
            ...
            if (password_verify($password, $hashed_password)) {
                ...
                // Determine the type of item (Product or Service)
                $itemType = $_POST["item_type"];

                // Construct appropriate queries based on item type
                if ($itemType == "product") {
                    // Add the product to deleted products database
                    $copyQuery = "INSERT INTO DeletedProducts SELECT * FROM Products WHERE ProductID = '$itemID';";
                    $deleteQuery = "DELETE FROM Products WHERE ProductID = '$itemID'";
                    $successMessage = "Product deleted successfully!";
                    $errorMessage = "Error deleting product: ";
                } elseif ($itemType == "service") {
                    // Add the service to deleted services database
                    $copyQuery = "INSERT INTO DeletedServices SELECT * FROM Services WHERE ServiceID = '$itemID';";
                    $deleteQuery = "DELETE FROM Services WHERE ServiceID = '$itemID'";
                ...
                $message = "Incorrect password! Item not deleted.";
                $popupClass = "error-popup";
            }
        }
    }
